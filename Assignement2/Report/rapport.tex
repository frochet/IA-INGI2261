	...
	\section{Search Algorithms}
		...
		\subsection{A* versus uniform-cost search}
			\subsubsection{Question 1: Give a consistent heuristic for this problem. Prove that it is admissible}
				...
				Lets consider a node and the goal, there is two possible cases:\\ 
				Either, we go in the direction of the goal following one of the axes and $$ h(n) = c(n,a,n') + h(n') $$
				Either, we don't move towards the goal and thus we get $$ h(n) < c(n,a,n') + h(n) $$
				In one case of the other, the inequality $$ h(n) \leq c(n,a,n') + h(n) $$ is respect and thus, the heuristic is consistent.\\
				As said earlier, the heuristic is consistent and so, it is admissible.
