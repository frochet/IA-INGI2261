...
In order to represent the problem, we will declare two variables per package. $I_{i}$, that is set to \texttt{True} if the package $i$ is installed and $V_{i}$, that is set to \texttt{True} if the package is a virtual package.\\
...
\begin{itemize}
  ...
  \item Provides : A provides B : $I_{A} \Rightarrow V_{B}$
\end{itemize}
