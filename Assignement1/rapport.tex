\ref{•}\documentclass[a4paper,10pt]{article}
...
\begin{document}
	...
	\subsection{Both breadth_first_graph and depth_first_search are making a call to the same function. How s their fundamental difference implemented}
	...
	There may be many situation where symmetrical states occurs. The consequences of this in the treatment of this problem	is that we me build several times the same tree and explore it as many times. It would constitute a useless usage of memory and a loss of time considering it generates and analyses the redundant states before reaching the goal state.
	...
	For this particular project we'll use the \{breath first or iterative deepening method SELON CE QU'ON CHOISIT DEMAIN EN SALLE INTEL JE SAIS PAS A QUELLE HEURE\}. 
	The best way to avoid twins states, is the breath first for it is the only one (with the uniform cost search) among the strategy proposed at question one that cares about avoiding treating multiple times the same state.
